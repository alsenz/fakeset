...
load YAML files
  ...
  → expand_field_variants      (heterogeneous → DenseUnion; case-3 ref+variants → cross-product for lowering; same-type → per-row marker)
...
| `models.rs` | All data types (`SyntheticDataset`, `Field`, `Include`, `ImportSpec`, `RingBounds`, `Schema`, `UnionCase`, …) and `FieldType` (incl. the internal `Union` marker — VAR-1). Also `resolve_distributions`, `eligible_linked_rows`, the list-link visitor, and lattice-traversal helpers. *Gotcha:* `Field` is not `PartialEq` (so `FieldType::Union` is a marker + `Field::union_cases`, not a data-carrying variant). |
...
| `expand_variants.rs` | Route `type: variant` fields (`expand_field_variants`). **Heterogeneous** → `lower_heterogeneous_unions` → `FieldType::Union` + `union_cases` (VAR-1). **Case-3** (`ref` + `variants`) → cross-product into `dataset.variants` for lowering. **Same-type, no ref** → `finalize_variant_fields` gives the field a unified concrete type but keeps its `variants` for per-row generation (VAR-UNIFY Phase 2). `is_heterogeneous`/`infer_field_type`/`unified_variant_type` (shared predicates, also used by `validate.rs`/`generator.rs`) |
...
| `plan.rs` | `build_plan` — row counts, lower cover groups, ring assignment, inherited-field wiring, collect targets → `ExecutionPlan` / `ExecutionStep`. `lower_member_variants` — lowers leaf-member tagged unions (constraint-bearing case-3 `ref` + `variants`) into case-members + `ExclusionGroup`s (skips members that are also parents). `subdivide_for_pinned_variants` — VAR-SPECIALIZE S4c `preserve_marginal`: subdivides segments by a pinned variant's cases via 2-D IPF + Gale–Hoffman feasibility check. (Plain same-type field variants generate per-row — no planner step.) |
...
| `specs/done/VAR-SPECIALIZE.md` (+ `-impl.md`) | **COMPLETE (S1–S5)** — child specialisation of parent fields. **Three cases:** (2) generator-domain (`value:`/`one_of:` overrides `generator:`); (1) variant-subset (`one_of:` restricts a parent union; open Option A/B fork); (3 — **NEW, the VAR-UNIFY U4 unblocker**) constraint-bearing variant carrier on a ref'd field (`ref` + `variants:` → each case lowers with the ref + value, entering segmentation). Unifying model: value-source is one **generator spectrum** (type-default ≻ `generator` ≻ `one_of` ≻ `value`), merge keeps the **richest carrier** + intersects supports (`merge(Variant, one_of) = Variant[subset] ≠ one_of`). Case 3 is required because `FieldVariant` has no `refs`, so top-level variants' constraint-bearing role (e.g. `variant_pruned_by_segment`) can't be expressed as a field variant — blocking U4. PR breakdown in `specs/done/VAR-SPECIALIZE-impl.md`; resolves the "Generator-plus-value should specialise, not conflict" known limitation |
...
- **Variant roadmap — COMPLETE.** Interleaved sequence as executed: `S1` ✅ → `S3` ✅ → `VAR-UNIFY U4–U6` ✅ → `S2` ✅ → `S4` ✅ → `S5` ✅. Only deferral: multi-level marginal-pinning.
  - **VAR-UNIFY — COMPLETE (U1–U6).** `flatten` output primitive (+ `flatten_strategy`) [U1–U3]; top-level `variants:` retired [U4]; same-type field variants generate **per-row** [U5]; `plan_variant_steps` + `CombineVariantBatches` deleted [U6]. The cross-product (`VariantSchema`/`SyntheticDataset.variants`) is **kept** — case-3 rides it into lowering (partial-deletion finding). **Subsumes VAR-1-OUTPUT-FLAG.** See `specs/done/VAR-UNIFY-impl.md`.
