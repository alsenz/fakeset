...
| `expressions.rs` | `pull_down_expression_deps`, `extract_identifiers` (AST/dependency analysis) |
| `expr_analysis.rs` | **All expression-column machinery** (EXPR-RELOCATE): **placement** (`classify_expression_placement` — staging vs assembly tier), **evaluation** (`evaluate_expression_fields`/`evaluate_expressions` — the DataFusion CTE chain; the one place expression columns are materialised), **static analysis without executing rows** (`infer_expression_types` via the planned `DataFrame` schema; `infer_expression_bounds` via `ExprIntervalGraph` forward eval → `rigid_support`), and **coercion** (`types_compatible`/`cast_computed_columns`). `scalar_input_context` builds the derive context from a member's own resolved fields |
...
| `constraints.rs` | `FieldConstraints`, `Satisfiable`, `Merge`, `Reconciled`/`reconcile`/`reconcile_rigid`, `NumericInterval`, `validate_field_constraints`. **Value-source spectrum** (VAR-SPECIALIZE + EXPR-RELOCATE): `expression ≻ value ≻ one_of ≻ generator ≻ type-default`. The malleable sources `merge` (tightest wins, supports intersect; `value`+`generator` is *not* a conflict). `expression` is the tightest and **rigid** — its derived interval is `rigid_support`, which a restriction can only be *verified* against, never narrow. **`reconcile` is the single combinator** (rigid case + the malleable `merge` core; `merge` is used directly only in pure-malleable contexts like `rewrite` specialisation). `one_of` = finite-set support selector; `case_overrides` carries `constrain_cases` deltas through merge (concat; ignored by pruning) |
...
| `executor.rs` | `execute` — interprets the plan; staging node generation, witness generation, assembly (folds the per-edge **junction** into lists, evaluates content-expressions, stashes the junction for collect), segment-atom pipeline (`generate_segment_atom_batch` with the import→precomputed→fresh→**computed** column sources via `compute_shared_columns` + `project_parent_columns_from_atom` + `project_member_columns`), `AccumulateToLinked` (sources the junction). Import branch: loads `ImportIndex`, ring filter, synthetic fields. All DataFusion/Arrow batch ops. (Expression placement/eval/typing lives in `expr_analysis.rs`; write-time output encoding in `output.rs`.) |
...
3. **A field's value-source is one spectrum**, tightest-first: `expression` ≻ `value` ≻ `one_of` ≻ `generator` ≻ type-default. The malleable sources (`generator`/`one_of`/`value`) *merge* — tightest wins, supports *intersect* (`FieldConstraints::merge`). `expression` (EXPR-RELOCATE) is the tightest and is **rigid** — it authors the value, carrying a derived `rigid_support` interval that a restriction can only be *verified* against (contained → keep, disjoint → prune, partial/underivable → hard error), never narrow. `FieldConstraints::reconcile` is the single combinator (rigid case + the malleable `merge` core). `value`+`generator` is **specialisation, not conflict**. `merge(Variant, one_of) = Variant[subset]` — the carrier (cases + ratios) survives, **renormalised** over the subset; it is *not* a flat uniform `one_of`. (Un-renormalised subset weights silently dump the tail mass on the last case.)
...
| `specs/done/EXPR-RELOCATE.md` (+ `-impl.md`) | **COMPLETE (PR1–PR4 + tidy-up)** — expression evaluation relocated from terminal emit into the materialisation pipeline, **placed by dependency** (`expr_analysis::classify_expression_placement`: staging-tier vs assembly-tier). An expression can **author a ref'd/shared column** projected up the lattice ("pin a ref'd field with an expr"): a member's `ref`+`expression` becomes the **computed** atom column-source (`generate_segment_atom_batch` → `compute_shared_columns`). Gated by the **constraint merge algebra** — `expression` is the tightest point on the value-source spectrum carrying a **rigid** derived interval (`FieldConstraints.rigid_support`, from `ExprIntervalGraph` forward eval); `reconcile` verifies a restriction against it (contained→keep / disjoint→prune / partial-or-underivable→error). **Content-expressions** evaluate per-edge at the assembly **junction**; an outer-dependent one can be **`collect`ed** into a linked `[]float` (one unified per-edge collect path). Lifted the content-expression gate that LIST-NORM rollups and PROJECT-FIELD's derivation half waited on (both now work). Expression placement/eval/typing lives in `lib/expr_analysis.rs`. The bigger "whole pipeline as DataFusion projections" rewrite remains a faint *possible* direction only (`specs/EXPR-MOONSHOT.md`), not a target |
| `specs/EXPR-MOONSHOT.md` | **Blue-sky stub — a *possible* direction only, not a target** (treat with heavy salt). The far end EXPR-RELOCATE (`specs/done/`) gestured at: all per-row column *value* production as a **chain of DataFusion projections** (`staging → linked → assembly`, iterated, scoped by YAML order). Combinatorial/numeric **planning kept outside** as small **per-atom parameters** (row counts, segment masks, range bounds, ratios) — *not* a full N-row frame — injected as scalar context; rows expanded **inside** by `volatile UDF + expr`. Centrepiece: a big inside/outside mechanism table. Load-bearing insight **ref = CSE** ("author once, fan out" free from the optimiser). **Strangler migration only** — one mechanism at a time, gated on `cargo test` + `pytest`; the parameter/projection seam keeps a half-migrated system shippable. The split (not "UDF-everything") is what enables incremental liberation of generator-arg parameterisation |
