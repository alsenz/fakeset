...
) -> (Vec<LowerCoverMember>, Vec<ExclusionGroup>) {
    let mut lowered: Vec<LowerCoverMember> = Vec::new();
...
fn plan_lower_cover_group_steps(
    ...
    // (The own-range check for expression-authored fields — scenario 1 — is done once where the
    // rigid support is derived, in `segment::lower_cover_field_constraints`.)
